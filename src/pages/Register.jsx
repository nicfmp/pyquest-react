    ...
    try {
      const res = await fetch(`${API_URL}/api/register`, {
        ...
        body: JSON.stringify({
          ...
          senha: form.password,
          dataNascimento: form.birthDate, // já vem como YYYY-MM-DD do <input type="date">
        }),
      })
      ...
      setErrors({ email: 'Não foi possível conectar ao servidor. Ele está rodando?' })
      setStatus('idle')
    }
  }
    ...
    <div className="flex min-h-screen items-center justify-center bg-paper px-6 py-12">
      <div className="w-full max-w-[400px]">
        ...
        <div className="rounded-2xl border border-line bg-paper-soft p-8 shadow-[0_20px_50px_-24px_rgba(22,29,24,0.25)]">
          <h1 className="mb-1 font-display text-[26px] font-bold text-ink">Crie sua conta</h1>
          <p className="mb-7 text-[14px] text-ink-soft">Comece sua jornada em Python gratuitamente, sem cartão de crédito.</p>

          <form onSubmit={handleSubmit} noValidate>
            <label htmlFor="username" className="mb-1 block text-[13px] font-medium text-ink">Nome de usuário</label>
            <input
              id="username" name="username" type="text" autoComplete="username"
              value={form.username} onChange={handleChange} placeholder="ex: ana_python"
              className={`mb-1 w-full rounded-[10px] border bg-paper px-3.5 py-[10px] text-[14px] text-ink outline-none transition-colors focus:border-forest ${errors.username ? 'border-red-400' : 'border-line'}`}
            />
            {errors.username && <p className="mb-3 text-[12.5px] text-red-500">{errors.username}</p>}
            ...
            />
            <p className="mb-1 text-[11.5px] text-ink-soft">Usada apenas para confirmar sua identidade caso precise redefinir a senha.</p>
            ...
            <label htmlFor="confirmPassword" className="mb-1 block text-[13px] font-medium text-ink">Confirmar senha</label>
            <input
        ...
        <p className="mt-6 text-center text-[13.5px] text-ink-soft">
          Já tem conta?{' '}
          <Link to="/login" className="font-medium text-forest hover:underline">Entrar</Link>
        </p>
      </div>
    </div>
  )
